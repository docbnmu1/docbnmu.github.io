...
var preCreatorScript =[creatorJqueryFile,"/siteforms/appcreator/live/common/js/form.js","/siteforms/appcreator/live/common/js/generatejs.js","/siteforms/appcreator/live/common/js/searchableInput.js","/siteforms/appcreator/live/common/js/app.js"];//NO I18N
...
function loadPreCreatorScripts() {//creator js
    ...
    if(preCreatorScript.length==0) {
        spcLoadScript(loadScriptNow,true);
    }else{
...
function spcLoadScript (src,load) {
    var script = document.createElement('script');
    if(load){
        if(this.addEventListener){
            script.onload = function(){loadCreatorScripts();};//check call sites script
        }
        else{
            script.onreadystatechange = function(){
                if(v(["loaded","complete"],this.readyState)>-1){
                    this.onreadystatechange= null;
                    loadCreatorScripts();//check call sites scrip
                }
            }
        }
    }
...
resizeElements = function(eve){
    if((typeof(responsiveTheme)!="undefined" && responsiveTheme==true) ||(typeof(window.parent.responsivePreview)!="undefined" && window.parent.responsivePreview=="true") ){
        ...
        if(banner ){
            var prevParWidth=banner.parentNode.clientWidth;
            var prevParHeight=banner.parentNode.clientHeight;
            banner.style.width="100%";
            banner.style.height="auto";
            banner.parentNode.style.width="100%";
            banner.parentNode.style.width=banner.parentNode.clientWidth+"px";
            banner.parentNode.style.height=Math.floor(banner.parentNode.clientWidth*(prevParHeight/prevParWidth))+"px";
            ...
            if(overlay){
                ...
                overlay.style.width=banner.parentNode.clientWidth+"px";
                overlay.style.height=banner.parentNode.clientHeight+"px";
